...
export default function OijPage() {
  ...
  const [filters, setFilters] = useState({ province: '', canton: '', district: '', crime: '', modality: '', targetCategory: '', targetType: '' });
  ...
  const reset = () => setFilters({ province: '', canton: '', district: '', crime: '', modality: '', targetCategory: '', targetType: '' });
  return <div className="mx-auto flex w-full max-w-6xl flex-col gap-6">
    ...
    {meta && data && <>
      <div className="grid gap-4 rounded-xl border bg-card p-4 sm:grid-cols-2 lg:grid-cols-4">
        <div className="grid gap-2"><Label htmlFor="oij-province">Provincia</Label><select id="oij-province" className={selectClass} value={filters.province} onChange={e => setFilters({ ...filters, province: e.target.value, canton: '', district: '' })}><option value="">Todas las provincias</option>{data.options.provinces.map(p => <option key={p}>{p}</option>)}</select></div>
        <div className="grid gap-2"><Label htmlFor="oij-canton">Cantón</Label><select id="oij-canton" className={selectClass} value={filters.canton} disabled={!filters.province} onChange={e => setFilters({ ...filters, canton: e.target.value, district: '' })}><option value="">{filters.province ? 'Todos los cantones' : 'Selecciona una provincia'}</option>{data.options.cantons.map(p => <option key={p}>{p}</option>)}</select></div>
        <div className="grid gap-2"><Label htmlFor="oij-district">Distrito</Label><select id="oij-district" className={selectClass} value={filters.district} disabled={!filters.canton} onChange={e => setFilters({ ...filters, district: e.target.value })}><option value="">{filters.canton ? 'Todos los distritos' : 'Selecciona un cantón'}</option>{data.options.districts.map(p => <option key={p}>{p}</option>)}</select></div>
        ...
        <div className="grid gap-2"><Label htmlFor="oij-modality">Modalidad</Label><select id="oij-modality" className={selectClass} value={filters.modality} onChange={e => setFilters({ ...filters, modality: e.target.value })}><option value="">Todas las modalidades</option>{data.options.modalities.map(p => <option key={p}>{p}</option>)}</select></div>
        <div className="grid gap-2"><Label htmlFor="oij-target-category">Objetivo</Label><select id="oij-target-category" className={selectClass} value={filters.targetCategory} onChange={e => setFilters({ ...filters, targetCategory: e.target.value, targetType: '' })}><option value="">Persona/vivienda/vehículo/edificación</option>{data.options.targetCategories.map(p => <option key={p}>{p}</option>)}</select></div>
        <div className="grid gap-2"><Label htmlFor="oij-target-type">Tipo específico</Label><select id="oij-target-type" className={selectClass} value={filters.targetType} disabled={!filters.targetCategory} onChange={e => setFilters({ ...filters, targetType: e.target.value })}><option value="">{filters.targetCategory ? 'Todos' : 'Selecciona un objetivo'}</option>{data.options.targetTypes.map(p => <option key={p}>{p}</option>)}</select></div>
      ...
      <Bars title="Modalidad del hecho" rows={data.byModality.slice(0, 15)} />
      <Bars title="Objetivo del delito (persona/edificación/vehículo)" rows={data.byTargetType.filter(r => r.name !== 'DESCONOCIDO').slice(0, 15)} />
      ...
      <Card><CardHeader><CardTitle>Comparación entre distritos</CardTitle></CardHeader><CardContent><div className="max-h-96 overflow-auto"><table className="w-full text-left text-sm"><thead className="sticky top-0 bg-card"><tr className="border-b"><th className="p-3">Provincia</th><th className="p-3">Cantón</th><th className="p-3">Distrito</th><th className="p-3 text-right">Registros</th><th className="p-3 text-right">% de la selección</th></tr></thead><tbody>{data.districts.slice(0, 300).map(r => <tr key={`${r.province}/${r.canton}/${r.district}`} className="border-b"><td className="p-3">{r.province}</td><td className="p-3">{r.canton}</td><td className="p-3">{r.district}</td><td className="p-3 text-right tabular-nums">{number(r.count)}</td><td className="p-3 text-right">{(100 * r.count / data.total).toFixed(1)}%</td></tr>)}</tbody></table></div>{!data.total && <p className="py-4 text-sm text-muted-foreground">No hay registros que coincidan con los filtros.</p>}{data.districts.length > 300 && <p className="pt-2 text-xs text-muted-foreground">Mostrando los 300 distritos con más registros. Filtra por provincia/cantón para ver el resto.</p>}</CardContent></Card>
